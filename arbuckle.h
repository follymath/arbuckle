...
// adapted from https://github.com/fredrik-johansson/arbcmath
// TODO just uses double complex helpers as a crutch to start, but should be optimized away
...
#define CHECK_FINITE(v, argc) { \
    if (argc > 0 && !isfinite(v[0]) || !isfinite(v[1])) { \
        v[argc + 0] = D_NAN; \
        v[argc + 1] = D_NAN; \
        return; \
    } \
    if (argc > 1 && !isfinite(v[2]) || !isfinite(v[3])) { \
        v[argc + 0] = D_NAN; \
        v[argc + 1] = D_NAN; \
        return; \
    } \
    if (argc > 2 && !isfinite(v[4]) || !isfinite(v[5])) { \
        v[argc + 0] = D_NAN; \
        v[argc + 1] = D_NAN; \
        return; \
    } \
    if (argc > 3 && !isfinite(v[6]) || !isfinite(v[7])) { \
        v[argc + 0] = D_NAN; \
        v[argc + 1] = D_NAN; \
        return; \
    } \
...
static __inline__ void arbuckle_ ## cfun \
(double* v) \
{ \
    acb_t in1, out1; \
    int argc = 1; \
    double complex z1 = MAKE_COMPLEX(v[0], v[1]); \
...
{ \
    ...
    int argc = 2; \
    double complex z1 = MAKE_COMPLEX(v[0], v[1]); \
    double complex z2 = MAKE_COMPLEX(v[2], v[3]); \
    double complex result; \
    CHECK_FINITE(v, argc) \
    _acb_init_set_c(in1, z1); \
...
{ \
    ...
    double complex z1 = MAKE_COMPLEX(v[0], v[1]); \
    double complex z2 = MAKE_COMPLEX(v[2], v[3]); \
    double complex z3 = MAKE_COMPLEX(v[4], v[5]); \
    double complex result; \
    CHECK_FINITE(v, argc) \
    _acb_init_set_c(in1, z1); \
    _acb_init_set_c(in2, z2); \
...
{ \
    ...
    double complex z2 = MAKE_COMPLEX(v[2], v[3]); \
    double complex z3 = MAKE_COMPLEX(v[4], v[5]); \
    double complex z4 = MAKE_COMPLEX(v[6], v[7]); \
    double complex result; \
    CHECK_FINITE(v, argc) \
    _acb_init_set_c(in1, z1); \
    _acb_init_set_c(in2, z2); \
